                            ...
                            {!! $product->p_description !!}
                             ...
                             <tbody>
                              @isset($pVariant)
                                @foreach($pVariant as $pv)
                                 <tr id="{{ $pv->id }}">
                                 <td><div  style="background-color:{{ $pv->colorName['color_code'] }};width:30px;height:30px;"></div></td>
                                 <td>{{ isset($pv->color_vary_price)?$pv->color_vary_price:"Price Included" }}</td>
                                 <td>
                                   @isset($pv->getGallaryImage)
                                       @foreach($pv->getGallaryImage as $img)
                                        <img src="{{ url('product/product-gallary') }}/{{ $img['image'] }}" style="height:150px;width:100px;">
                                       @endforeach
                                   @endisset
                                 </td>
                                 <td>
                                   <button class="btn btn-danger btn-sm removeGallary"><i class="far fa-trash-alt"></i></button>
