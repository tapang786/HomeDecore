...
<div class="card">
   ...
   <div class="card-body">
          <div id="accordion">
            <?php if(!session()->has("addVariantStyle") && !session()->has('setFabric')){ ?>
              <div class="card">
              ...
              <?php } ?>
              @if(session()->has('setFabric') || !session()->has('addVariantStyle'))
               <div class="card">
                <div class="card-header pvarient" id="headingFab">
                   <h5 class="mb-0">
                     <button class="btn btn-link" data-toggle="collapse" data-target="#collapseFab" aria-expanded="true" aria-controls="collapseFab" type="button">
                      Fabric Setting
                     </button>
                   </h5>
                 </div>
                 <div id="collapseFab" class="collapse" aria-labelledby="headingFab" data-parent="#accordion">
                   <div class="card-body">
                     <h6>Select Product Fabric</h6><hr>
                     @if(session()->has('msg'))
                      <p class="alert-success alert p-1">{{ session('msg') }}</p>
                     @endif
                     <form id="setFabric" method="POST" enctype="multipart/form-data" action="{{ route('admin.set-product-fabric') }}" >
                         @csrf
                         <input type="hidden" name="pid" value="{{ isset($pid)?$pid:'' }}">
                          <input type="hidden" name="clrid" value="{{ session()->has('clrid')?session('clrid'):'' }}">
                     <div class="row">
                       @isset($fabric)
                        @foreach($fabric as $fab)
                         <div class="col-sm-1 float-left">
                         <div class="clearfix"></div>
                          <div style="margin-left:12px;" class="text-center">
                             @if(session()->has('colorid') && $clr->id==(int)session('colorid'))
                             {{ $fab->name }}
                          <input type="radio" name="fabric" value="{{ $fab->name }}" class="form-check fabric" checked="">
                          @else
                          {{ $fab->name }}
                          <input type="radio" name="fabric" value="{{ $fab->name }}" class="form-check fabric">
                          @endif
                          </div>
                          </div>
                          @endforeach
                          @endisset
                            <div class="col-sm-12 text-right">
                                  <button class="btn btn-success btn-sm">Apply Fabric</button>  
                           </div>
                     </div>
                    </form>
                   </div>
               ...
               @endif
               @if(!session()->has("addVariantStyle") && !session()->has('setFabric') && !session()->has('setFabric'))
              <div class="card">
            ...
            @if (!session()->has('setFabric'))
              <div class="card">
                ...
                <div id="collapseThree" class="collapse {{ session()->has("addVariantStyle") && !session()->has('setFabric')?'show':'' }}" aria-labelledby="headingThree" data-parent="#accordion">
                  <div class="card-body">
                    @if(!session()->has("addVariantStyle") && !session()->has('setFabric'))                
                    <div class="row">
                        <div class="col-sm-12">
                       ...
                       <div class="row">
                        ...
                        <div class="col-sm-3">
                         ...
                         <div class="col-sm-4">
                          ...
                          <div class="col-sm-2">
                            <label>Make Default Style</label>
                             <input type="checkbox" name="defaultNeck" class="form-check" value="default style">
                         </div>
                             <div class="col-sm-1  text-right">
                              @php $img=isset($stedit)?json_decode($stedit->image,true):''  @endphp
                             <img src="{{ url('') }}/{{isset($img['image'])?$img['image']:'V.jpg'}}" style="height:100px;width:50px;">
                            </div>
 ...
 <option value="{{ isset($stedit)?$stedit->product_style_customization_id :'' }}" selected="">{{ isset($stedit)?$stedit->getStyleName['style_group_name']:'Select sleeve style' }}</option>
                         ...
                         <div class="col-sm-4 ">
                          ...
                          <div class="col-sm-2  text-right">
                              @php $img=isset($stedit)?json_decode($stedit->image,true):''  @endphp
                             <img src="{{ url('') }}/{{ isset($img['rhand'])?$img['rhand']:'right-hand.jpg'}}" style="height:100px;width:70px;">
                         ...
                         <div class="col-sm-2">
                            <label>Make Default Style</label>
                             <input type="checkbox" name="defaultSleeve" class="form-check" value="default style">
                         </div>
                           <div class="col-sm-2  text-right">
                            <img src="{{ url('') }}/{{isset($img['lhand'])?$img['lhand']:'left-hand.jpg'}}" style="height:100px;width:70px;">
              ...
              <option value="{{ isset($stedit)?$stedit->product_style_customization_id:'' }}" selected="">{{ isset($stedit)?$stedit->getStyleName['style_group_name']:'Select length style' }}</option>
                         ...
                         <div class="col-sm-4">
                             ...
                             <div class="col-sm-2">
                            <label>Make Default Style</label>
                             <input type="checkbox" name="defaultLength" class="form-check" value="default style">
                           </div>
                             <div class="col-sm-1  text-right">
                                @php $img=isset($stedit)?json_decode($stedit->image,true):''  @endphp
                             <img src="{{ url('') }}/{{ isset($img['image'])?$img['image']:'bottom.jpg'}}" style="height:100px;width:70px;">
                         </div>
                         <div class="col-sm-2 text-right">
                             <button class="btn btn-success btn-sm">Upload</button>
                         </div>                
