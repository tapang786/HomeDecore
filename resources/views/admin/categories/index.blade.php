                ...
                <tbody class="c">
                      @isset($cat)
                        ...
                        <tr id='{{ $item['id'] }}'>
                          ...
                          <td>{{ isset($item['p_cat_name']) ? Str::ucfirst($item['p_cat_name']) : 'No Parent' }}</td>
                          <td>
                            ...
                            @endif
                          <td>
                            <a class="btn btn-xs btn-success" href="{{ url('/category') }}/{{ $item['slug'] }}">
         ...
         $.ajax({
            ...
            success:function(data)
            {
              if(data!='You have already added this category')
              {
                ...
                var row='<tr id='+d.id+'><td></td><td>'+d.name+'</td><td>'+d.cid+'</td><td>'+sts+'</td><td>'+action+'</td></tr>';

                $(".c").prepend(row);
                $("#pc").append('<option'+d.id+'>'+d.name+'</option>');
                $("#addcat")[0].reset();
                swal("Added");
              }else{
                 swal("Sorry!! ",data);
